...
> - Điều này cho em biết JavaScript đang làm gì khi trang web được tải?
    JavaScript thực hiện thực thi khi trang web tải và hiển thị thông báo vào Console
---
...
> a) JavaScript chạy ở đâu? (Trình duyệt / Server / Cả hai?)
    Cả trình duyệt và trên server
> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
>
> - HTML: dùng để tạo cấu trúc và nội dung của trang web
> - CSS: dùng để thiết kế giao diện và định dạng (màu sắc, bố cục).
> - JavaScript: dùng để tạo sự tương tác và xử lý logic trên trang web. 
...
>   - `number` là: kiểu dữ liệu trả về số (vd 1, 2, 3, ...)
>   - `string` là: kiểu dữ liệu trả về kí tự (Trang, ...)
...
> - Dòng log hiển thị gì sau khi em sửa thông tin?
    Xin chào, mình là Trang, năm nay mình 21 tuổi.
...
> - Điều thú vị nhất em vừa khám phá được về console là gì?
    báo kết quả rất nhanh
    cho phép chạy thử JavaScript
...
nếu điều kiện đưa ra thỏa mãn đúng yêu cầu sẽ thực thi ngay lập tức
nếu điều kiện không thỏa mã sẽ bỏ qua xét trường hợp tiếp
...
>   - Tham số (parameters): 8, 7, 9
>   - Thân hàm (body): phần trong { }
>   - Giá trị trả về (return): return avg
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
    không mất công viết lại code nếu phải dùng nhiều lần
    dễ tìm lỗi sai và sửa lỗi
    giảm ngắn code giúp dễ hiểu hơn
---
...
> - Sự kiện `"input"` khác gì so với `"click"`?
  input xảy ra khi gõ hoặc thay đổi nội dung ổ input
...
>  biểu diễn trang HTML dưới dạng sơ đồ dễ nhìn hơn
...
> - Điểm giống nhau về chức năng giữa 2 đoạn code trên là gì?
  đều dùng xử lí việc tương tác với trang web
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, đây là JavaScript thuần
`addEventListener` vs `.on`)? đây là jQuery
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. Thao tác DOM dễ dàng hơn
>   2. Xử lý sự kiện và hiệu ứng nhanh hơn
...
> 1. Sau buổi lab, em tò mò nhất về phần nào của JavaScript/DOM?
  Cách JavaScript tạo tương tác động trên trang web
> 2. Em muốn tự làm thêm tính năng gì trên trang web (vd: bộ đếm, đổi theme, pop-up, mini game, …)?
  Nút đổi theme sáng/tối hoặc bộ đếm số lần click
